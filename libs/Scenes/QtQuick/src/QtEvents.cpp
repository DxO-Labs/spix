...
void QtEvents::mouseDown(Item* item, Point loc, MouseButton button, KeyModifier mod, bool eventToItem)
{
    QPointF windowLoc;
    ...
    if (eventToItem) {
        ...
        QMouseEvent* event = new QMouseEvent(
            QEvent::MouseButtonPress, qtlocalPoint, eventCausingButton, activeButtons, qtmod);
        QGuiApplication::postEvent(qtitem->qquickitem(), event);
    } else {
        QMouseEvent* event
            = new QMouseEvent(QEvent::MouseButtonPress, windowLoc, eventCausingButton, activeButtons, qtmod);
        QGuiApplication::postEvent(window, event);
    }
}

void QtEvents::mouseUp(Item* item, Point loc, MouseButton button, KeyModifier mod, bool eventToItem)
{
    QPointF windowLoc;
    auto window = getWindowAndPositionForItem(item, loc, windowLoc);
    if (!window)
    ...
    if (eventToItem) {
        const QPointF qtlocalPoint(loc.x, loc.y);
        QMouseEvent* event = new QMouseEvent(
            QEvent::MouseButtonRelease, qtlocalPoint, eventCausingButton, activeButtons, qtmod);
        QGuiApplication::postEvent(qtitem->qquickitem(), event);
    } else {
        QMouseEvent* event
